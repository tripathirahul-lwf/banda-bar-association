...
try {
    ...
    echo "Running Phase 12 Database Updates..." . PHP_EOL;

    // 1. Alter contact_messages if status doesn't exist
    try {
        $db->exec("ALTER TABLE `contact_messages` ADD COLUMN `status` ENUM('new', 'read', 'resolved', 'archived') DEFAULT 'new'");
        echo "SUCCESS: Added status column to contact_messages." . PHP_EOL;
    } catch (PDOException $e) {
        // Column might already exist
        echo "INFO: contact_messages status column check complete." . PHP_EOL;
    }
    ...
    ");
    echo "SUCCESS: system_settings table created." . PHP_EOL;
    ...
    ");
    echo "SUCCESS: audit_logs table created." . PHP_EOL;
    ...
    ");
    echo "SUCCESS: notifications table created." . PHP_EOL;
    ...
    $defaultSettings = [
        ...
        ['content', 'mission', 'हमारा मिशन अधिवक्ताओं की एकता और विधिक सहायता प्रदान करना है।', 'text', 1],
    ];

    $insertStmt = $db->prepare("
        INSERT INTO `system_settings` (`setting_group`, `setting_key`, `setting_value`, `value_type`, `is_public`)
        VALUES (?, ?, ?, ?, ?)
    ...
    echo "SUCCESS: Seeded default settings." . PHP_EOL;
    echo "Phase 12 Database Updates completed successfully." . PHP_EOL;
