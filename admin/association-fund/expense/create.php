...
$pageTitle = 'व्यय लेन-देन दर्ज करें (Record Expense Transaction)';
...
if ($db) {
    try {
        $categories = $db->query("SELECT * FROM association_fund_categories WHERE type = 'expense' AND status = 'active' ORDER BY name ASC")->fetchAll();
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($category_id <= 0) {
        $errors[] = 'व्यय की श्रेणी चुनना आवश्यक है।';
    }
    ...
    if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($errors)) {
            ...
            $attachment_filename = 'exp_' . uniqid() . '_' . time() . '.' . $ext;
            $upload_path = __DIR__ . '/../../../storage/financials/' . $attachment_filename;
            
            if (!move_uploaded_file($file['tmp_name'], $upload_path)) {
                $errors[] = 'व्यय प्रमाण दस्तावेज़ सहेजने में विफल।';
                $attachment_filename = null;
            }
        }
    }
    ...
    if (empty($errors) && $db) {
        try {
            ...
            // Auto-generate voucher number if blank and approved
            if (empty($voucher_no) && $status === 'approved') {
            ...
            $stmt = $db->prepare("
                INSERT INTO association_fund_transactions (
                    ...
                    description, voucher_no, attachment_path, status, created_by, 
            ...
            ");
            $audit->execute([$status, "Fund Expense Transaction $transaction_no ($amount) recorded as $status", $user_id]);

            $db->commit();
            setFlash('success', 'व्यय प्रविष्टि सफलतापूर्वक दर्ज कर दी गई है। (' . $transaction_no . ')');
            redirect('../transactions.php');

        } catch (PDOException $e) {
            if ($db->inTransaction()) $db->rollBack();
            error_log("Failed to create expense transaction: " . $e->getMessage());
...
<div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2 font-hindi">
    <h4 class="text-navy-custom fw-bold mb-0">व्यय प्रविष्टि दर्ज करें (Add Expense)</h4>
...
<div class="card p-4 border-0 shadow-sm font-hindi small mb-5">
    <form method="POST" action="create.php" enctype="multipart/form-data">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6 col-12">
                <label class="form-label text-secondary fw-semibold">व्यय श्रेणी (Expense Category) <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6 col-12">
                <label class="form-label text-secondary fw-semibold">व्यय राशि (Amount in ₹) <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6 col-12">
                <label class="form-label text-secondary fw-semibold">आदाता / प्राप्तकर्ता का नाम (Payee Name)</label>
                <input type="text" class="form-control form-control-sm" name="party_name" placeholder="उदा. वेंडर या सेवा प्रदाता का नाम">
            </div>
            ...
            <div class="col-md-6 col-12">
                <label class="form-label text-secondary fw-semibold">मैनुअल वाउचर संख्या (Voucher No.) <small class="text-muted">(यदि भौतिक बुक का प्रयोग हो)</small></label>
                <input type="text" class="form-control form-control-sm" name="voucher_no" placeholder="उदा. VOU-2026/001">
            </div>

            <div class="col-md-6 col-12">
                <label class="form-label text-secondary fw-semibold">व्यय बिल प्रमाण (Bill scan scan) <small class="text-muted">(PDF, JPG, PNG - Max 5MB)</small></label>
            ...
            <div class="col-12">
                ...
                <textarea class="form-control form-control-sm" name="description" rows="3" placeholder="उदा. कार्यालय स्टेशनरी क्रय हेतु भुगतान" required></textarea>
            </div>
